...
{
	extract($_POST);
	$qry="insert into gallerytype (name,detail,status) values ('$tname','$tdetail','$actstatus')";
...
<html lang="en">
  <head>
	...
	function validation()
	{
		...
		if(Name=="")
		{
			document.getElementById("sname").innerHTML="<b>Enter Gallerytype Name</b>";
			error=document.getElementById("sname").style.color="red";
			return false;
		}
		else if(!reg_name.test(Name))
		{
			document.getElementById("sname").innerHTML="<b>Check Gallery Name</b>";
			error=document.getElementById("sname").style.color="red";
			return false;
		}
		else
      ...
      <div class="app-title">
		...
		<center> 
			<form class="login-form col-sm-5" id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
			  <h3 class="login-head"><i class="fa fa-lg fa-fw fa-picture-o"></i>GALLERY TYPE ADD</h3>
			  <div class="form-group">
				<label class="control-label"> GALLERY NAME</label>
				<input class="form-control"  type="text" id="tname" name="tname" placeholder="Gallery Name" required autofocus><span id="sname"></span>
			  </div>
			   <div class="form-group">
				  <label for="comment">DETAIL</label>
				  <textarea class="form-control" rows="3" name="tdetail" id="tdetail" placeholder="Gallery Detail" required></textarea><span id="sdetail"></span>
				</div> 
				<div class="form-group">
