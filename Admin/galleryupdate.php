...
if(isset($_POST["btn1"]))
{
	...
	{
		$path="gallery/".$_FILES["timage"]["name"];
		move_uploaded_file($_FILES["timage"]["tmp_name"],$path);
	}
	$image=$_FILES["timage"]["name"];
	if($image)
	{
		$qry="Update gallery set gytid='$tgallerytype',name='$tname',detail='$tdetail',image='$path',status='$actstatus' where gid='$urlid'";
		$rs=$con->executequery($qry);
		header("location:galleryview.php");
	}
	else
	{
		$qry="Update gallery set gytid='$tgallerytype',name='$tname',detail='$tdetail',status='$actstatus' where gid='$urlid'";
		$rs=$con->executequery($qry);	
		header("location:galleryview.php");
 	}
	if(!$rs)
...
$qry1="select * from gallery inner join gallerytype on gallery.gytid=gallerytype.gytid where gid='$urlid'";
  ...
  <body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
      <div class="app-title">
		...
		<div class="row">
			<div class="col-md-5 col-sm-4 ml-10"></div>
			<div class="col-md-6 col-sm-12"><?php echo "<img src='$photo'; height='200px' width='250px;' class='img-responsive' >"; ?></div>
		</div>
		<center> 
			<form class="login-form col-sm-5" id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
			  ...
			  <div class="form-group">
				...
				while($r2=mysql_fetch_array($rs2))
						{?>
						<option value='<?php echo $r2[0]?>' <?php if($r2[0]==$gallerytype){echo 'selected=selected';}?>><?php echo $r2[1]?></option>
						<?php } ?>
				</select>
			  </div>
