...
if(isset($_POST["btn1"]))
{
	...
	for($p=0;$p<$count;$p++)
	{
	$qry1="update permission set uid='$user',mid='$checkbox1[$p]'  where pid='$id'";
		echo $qry1;
	...
	{
		$status="Update";
		header("location:permissionview.php");
	}
	else
	{
		$status="Not Update";
...
$qry="select * from permission where pid='$id'";
      ...
      <div class="app-title">
		...
		<center> 
			<form class="login-form col-sm-5" id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
			  <h3 class="login-head"><i class="fa fa-lg fa-fw fa-align-right"></i>PERMISSION UPDATE</h3>
			  <center>
			  ...
			  <div class="from-group ">
				...
				<?php
					$qry="select * from menusetup";
					$rs=$con->readrecord($qry);
					$status.="<table class='table  mt-2'>";
						$status.="<thead class='thead-dark'>";
							$status.="<tr>";
								$status.="<th scope='col'>Id</th>";
								$status.="<th scope='col'>Name</th>";
								$status.="<th scope='col'>Check</th>";
							$status.="</tr>";
						$status.="</thead>";
						$a=1;
							...
							$status.="<tbody>";
								...
								$status.="<td>$a</td>";
								$status.="<td>$row[1]</td>";
								$status.="<td><input type='checkbox' class='form-check-input' value='$row[0]' name='menu[]' id='menu'></td>";
								$status.="</tr>";
							$status.="</tbody>";	
							$a++;
							}
			  ...
			  <div class="form-group btn-container">
				<button class="btn btn-primary mt-2" id="btn1" name="btn1"><i class="fa fa-pencil-square-o fa-lg fa-fw"></i>UPDATE</button>
			  </div>
			</form>
