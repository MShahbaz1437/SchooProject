...
$con = new connectioninfo();
	if(isset($_POST["btn1"]))
	{
		...
		if($image)
		{
			$qry="update titlesetup set tname='$tname',logo='$path',temail='$temail',tcontact='$tcontact',tfax='$tfax',taddress='$taddress' where tid='$id'";
		}
		else
		{
			$qry="update titlesetup set tname='$tname',temail='$temail',tcontact='$tcontact',tfax='$tfax',taddress='$taddress' where tid='$id'";
		}
	...
	{
		extract($_POST);
		$qry="delete from classinfo where classid='$id'";
		$rs=$con->readrecord($qry);
		
	}
	$qry="select * from titlesetup where tid='$id'";
	$rs=$con->readrecord($qry);
  ...
  <body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
      <div class="app-title">
		...
		<center> 
		<?php echo "<img src='$logo' style='height:100px; width:150px;'/>";?>
			<form class="login-form col-sm-5" id="frm" name="frm" method="post" action="" enctype="multipart/form-data">
			  <h3 class="login-head"><i class="app-menu__icon fa fa-area-chart"></i>TITLE UPDATE</h3>
			...
			<div class="form-group">
				...
				<input class="form-control"  type="text" id="tcontact" name="tcontact" value="<?php echo $contact; ?>"  >
			</div>
			<div class="form-group">
				<label class="control-label">FAX</label>
				<input class="form-control"  type="text" id="tfax" name="tfax" value="<?php echo $fax; ?>"  >
			</div>			
			<div class="form-group">
